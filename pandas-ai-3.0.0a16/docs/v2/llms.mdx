...
description: "PandaAI supports several large language models (LLMs) that are used to generate code from natural language queries."
---
...
BambooLLM is the state-of-the-art language model developed by [PandaAI](https://pandas-ai.com) with data analysis in
...
PandaAI has also built-in support for [LangChain](https://langchain.com/) models.
...
PandaAI will automatically detect that you are using a LangChain LLM and will convert it to a PandaAI LLM.
...
_At this time, watsonx.ai does **not** support the PandaAI agent_.
...
PandaAI supports local models, though smaller models typically don't perform as well. To use local models, first host
