...
description: "A comprehensive guide on configuring, and using the PandaAI dockerized UI platform."
---

# Using the Dockerized Platform

PandaAI provides a dockerized client-server architecture for easy deployment and local usage that adds a simple UI for conversational data analysis. This guide will walk you through the steps to set up and run the PandaAI platform on your local machine.
...
1. Clone the PandaAI repository:
...
3. Edit the `.env` files and update the `PANDASAI_API_KEY` with your API key:
   ...
   Replace `YOUR_PANDASAI_API_KEY` with your PandaAI API key. You can get your free API key by signing up at [PandaAI](https://pandabi.ai).
...
- `server`: Builds and runs the PandaAI server.
- `client`: Builds and runs the PandaAI client interface.
